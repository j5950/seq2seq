...
train_data, val_data, test_data, src_vocab, trg_vocab = get_data(
    data_name='WMT14DE', batch_first=True)
...
def train(model, src_seqs, src_lens, trg_seqs, optimizer, criterion, is_valid):
    ...
    out = model(src_seqs_var, src_lens, trg_seqs_var) # B x L x V
    ...
    num_corrects = num_corrects.masked_select(labels.data.ne(trg_vocab.stoi['<pad>'])).sum()
